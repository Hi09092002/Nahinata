    ...
    },
    {
      ...
      groupContents: [
        ...
        },
        {
          ...
          commentary:
            'アセチル CoA が糖代謝と脂肪酸合成を結びつけていることが分かればよい。',
        },
        {
          detailInfo: '6',
          questionImg: [],
        ...
        },
        {
          ...
          answerImg: [],
          answer:
            'DNA は逆平行の二重らせん構造をとっており、二本鎖の DNA の間に塩基の部位を内部にしまうことができるので、塩基の損失や置換による遺伝子情報の変化・損失を抑え ることができる。また複製において誤った塩基を付加された場合、相補鎖の関係から DNA ポリメラーゼの持つ酵素によって除去されて正確な DNA 合成が行われる。複製が正確に行われたとしても突然変異による一塩基変異が起こりうるが、相補的な塩基情報を用いるこ とで修復は可能である。これより DNA は非常に安定しており、遺伝子情報を担う物質として適している。一方 RNA は一本鎖であるため、 DNA のような校正機構を持たず、変異が起こりやすい。また校正されないため変異が起こっても修復されることはなく変異は蓄積されていき、様々な性質を持つ変異ウイルスを生じやすい。コロナウイルスはプラス RNA 一本鎖ウイルスとして知られている。そのためセントラルドグマにおいて転写は行われないが、DNA を遺伝子情報として持つ細胞と同じく翻訳を行い 遺伝情報の複製を行なっていく。しかし前述の通り DNA による複製との比較より RNA からの翻訳は不正確であり変異が非常に起こりやすい。そのため変異型ウイルスが多く現れ得る。このことは昨今の報道での変異型ウイルスや、各国のワクチン開発が遅れている原因となっていると考えられる。',
          commentary: '',
        },
        {
          detailInfo: '8',
          ...
          answerImg: [],
          answer:
            'DNA は逆平行の二重らせん構造をとっており、二本鎖の DNA は相補的な塩基対間で形成される水素結合によって繋がっている。この繋がりは温度を上げることで切ることができ、これを変性という。また徐々に温度を下げていくことで水素結合が再形成されて 復元される。これらの作用を利用したのが PCR 法である。まず増幅したい領域の両端に相補的なプライマーDNA、鋳型のゲノム DNA、耐熱性を持つ DNA ポリメラーゼ、材料となる 4 種のヌクレオチドを混合する。そして温度を上げるとゲノム DNA は変性により一本鎖となる。その後徐々に温度を下げていくことでプライマーDNA とゲノム DNA の複製がお こる。次に DNA ポリメラーゼの至適温度まで温度上昇することでプライマーから DNA 合 成が起こっていく。この一連の操作を繰り返すことでプライマー間の DNA を特異的に増幅 することになる。このようにして生じた増幅断片を電気泳動にかける。そして検体において も同じようにした断片の泳動度と同じであればその断片は感染の疑いがあるといえる。こ のように DNA の特性を使い PCR 法は行われる。',
    ...
    },
    {
      ...
      groupContents: [
        ...
        },
        {
          ...
          answerImg: [],
          answer:
            'DNA は逆平行の二重らせん構造をとっており、二本鎖の DNA は相補的な塩基対間で形成される水素結合によってつながっている。この水素結合は温度を 上げることで切ることができる。これを DNA の変性という。また温度を徐々に下げることで元の塩基対 間の 水素結合が再形成される。これを DNA の復元 (アニーリング) という。アニーリングは塩基対が相補的であれば起こり、相補鎖は必ずしも元ペアであった DNA 鎖である必要はない。このメカニズムが応用されている例としては PCR 法が挙げられる。',
          commentary: '',
        },
        {
          detailInfo: '8',
    ...
    },
    {
      ...
      groupContents: [
        ...
        },
        {
          ...
          answerImg: [],
          answer:
            '目的は遺伝子組み換え生物等の使用等による生物多様性への悪影響を防ぐこと。 実習において、試験管内で組換えた DNA を持つプラスミドを大腸菌に導入する実験を行った。カルタヘナ法に基づき、実験室には「関係者以外立ち入り禁止」の表示があり、実習中は組み換え生物を外部に拡散させないために、実験室を密室にし、入退室の制限を行なった。使用したチップ等も廃棄前に不活化の措置を講ずるために専用の BOX に回収した。',
          commentary: '',
        },
        {
          detailInfo: '7',
      ...
      groupContents: [
        {
          ...
          answerImg: [],
          answer:
            '（1）アンフィンセンは、あるタンパク質に変性剤(尿素とβ―メルカプトエタノール)を加え、 水素結合とジスルフィド結合を切断する処理を行い、変性させた後、変性剤を取り除いた。 その結果、変性したタンパク質がもとの立体構造に自発的に折りたたまれた。 この実験から導かれた結論である「アミノ酸配列(一次構造)が高次構造を決定する」というドグマがアンフィンセンのドグマである。（2）細胞内では、タンパク質濃度がかなり高い。そのため、タンパク質の折りたたみ過程において、そのタンパク質の疎水性部分が他のタンパク質の疎水性部分と凝縮してしまったり、あるいは、他のタンパク質の持つ電荷の作用などにより、ネイティブな構造に折りたたまれな いことがある。これらの理由から、細胞内において必ずしも一次構造が高次構造を決定するとは限らない。',
          commentary: '',
        },
        {
          detailInfo: '2',
        ...
        },
        {
          ...
          questionImg: [],
          questionSentence:
            '（1）cAMPは受容体などと共役してはたらくアデニル酸シクラーゼによって合成され、細胞内のcAMP依存型プロテインキナーゼを活性化する。このような物質をなんというか。また、なぜこのような物質が必要なのか説明せよ。 （2）プロテインホスファターゼとは一般的にどのような反応を触媒する酵素か。 （3）ヒトにおいてエネルギー源が過剰な時、トリアシルグリセロールが合成されて脂肪組織 に蓄えられるが、この過剰なエネルギー源はどのようにして脂肪酸合成酵素の基質になるか説明しなさい。',
          choices: [],
          answerImg: [],
          answer:
            '（1）セカンドメッセンジャーという。 ホルモンや神経伝達物質などが、細胞表面の受容体に結合することで一連の反応を引き起こし、その結果として特異的な細胞応答が生じる。 この時、ホルモンや神経伝達物質と細胞への最終的な効果の間に介在する物質(セカンドメッセンジャー)が必要であるため。（2）タンパク質のリン酸化された残基を脱リン酸化する反応を触媒する酵素。（3）エネルギー源が過剰な時、細胞内 ATP 濃度が上昇し、TCA 回路はイソクエン酸からαケトグルタル酸への反応で阻害されるためにクエン酸が蓄積する。 これらのクエン酸は細胞質へ輸送された後、オキサロ酢酸とアセチル COA に分解される。 このように生成されたアセチル COA がカルボキシル化され、マロニル COA となる。 このようにして脂肪酸合成酵素の基質であるマロニル COA が得られる。',
          commentary: '',
        },
        {
          detailInfo: '4',
          ...
          answerImg: [],
          answer:
            'Km とは反応初速度が最大反応速度の半分の時の基質濃度のことであり、基質に対する酵素 の親和性の指標として用いられる。血糖値が低いときは、筋肉などの組織において、Km 値 が低く、グルコース濃度が低くてもグルコースを効率的にリン酸化できるヘキソキナーゼ が働き、一方で血糖値が高いときは、肝臓などの組織において、Km 値が高く、高濃度のグルコースを効率的にリン酸化できるグルコキナーゼが働く。',
          commentary: '',
        },
        {
          detailInfo: '5',
        ...
        },
        {
          ...
          answerImg: [],
          answer:
            'HIV ウイルスはヘルパーT 細胞内に侵入し、逆転写酵素を使い自らのゲノミック RNA から DNA を逆転写により合成する。その DNA をヘルパーT 細胞の持つ DNA に組み込み、転 写することで RNA を産出し、この RNA の一部から翻訳によりウイルス構造蛋白質が作られ、また、遺伝情報の担体として産出された RNA を使うことでウイルスが複製される。 HIV ウイルスの持つ逆転写酵素は正確性が低く、変異が入りやすいため、様々な性質を持った変異ウイルスが生じやすい。このことがワクチン開発を困難にしている。',
          commentary: '',
        },
      ],
    },
