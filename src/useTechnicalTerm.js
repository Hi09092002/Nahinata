...
export const useTechnicalTerm = () => {
  const [technicalTerm, setTechnicalTerm] = useState([
    ...
    },
    {
      ...
      explanation:
        '酵素の調節部位（アロステリック部位）に調節因子が可逆的に非共有結合で結合すると酵素のコンホメーションが変化し、酵素活性が上昇または低下することをアロステリック効果といい、このような制御を受ける酵素をアロステリック酵素allosteric enzymeと呼ぶ。酵素を合成・分解しないため短時間で酵素の活性を変化させることができる。アロステリック酵素はミカエリス-メンテンの式には従わず、v₀と[S]のプロットはシグモイド型（S字型）の曲線を描く。→酸素解離曲線？　アロステリック酵素の例：アスパラギン酸トランスカルバモイラーゼ',
    },
    {
    ...
    },
    {
      term: ['リン酸化酵素'],
      explanation:
        'kinaseと付く酵素はATPからリン酸を供与され基質をリン酸化する。ホスホリラーゼと呼ばれる酵素はATPではなく無機リン酸を付加させる酵素。',
    },
    {
      term: ['脱リン酸化酵素'],
    ...
    },
    {
      ...
      explanation:'特定の分子（抗原）を認識して特異的に結合し（→抗体）、遷移状態の基質と特異的に結合してこれを安定化する（→酵素）ようなタンパク質。人工酵素として利用されている。',
    },
    // {
    //   term:[''],
    //   explanation:'',
